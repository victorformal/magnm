...
export function CountdownTimerFr() {
  const [timeLeft, setTimeLeft] = useState("")

  useEffect(() => {
    const getOrSetExpiry = (): number => {
      const stored = localStorage.getItem("offer_expiry_fr")
      const now = Date.now()
      if (!stored || now > parseInt(stored)) {
        const expiry = now + 24 * 60 * 60 * 1000
        localStorage.setItem("offer_expiry_fr", expiry.toString())
        return expiry
      }
    ...
    const interval = setInterval(() => {
      ...
      if (remaining <= 0) {
        localStorage.removeItem("offer_expiry_fr")
